...
.name{font-weight:700}.contact a{display:block;color:var(--blue);text-decoration:none}.small{font-size:12px;color:var(--muted)}.status,.notes{width:100%;border:1px solid #c9c7c0;border-radius:3px;padding:6px;font:inherit;background:#fff}.status{min-width:125px}.notes{width:220px;min-height:54px;resize:vertical}.save{padding:6px 9px}.empty{padding:60px 28px;text-align:center;color:var(--muted)}.flash{position:fixed;right:20px;bottom:20px;background:var(--ink);color:#fff;padding:10px 14px;border-radius:7px;display:none}
@media(max-width:700px){header{align-items:flex-start;flex-direction:column}.stats{grid-template-columns:1fr}.stat{border-right:0;border-bottom:1px solid var(--line)}.filters{flex-direction:column}.filters input{min-width:0}.table-wrap{max-height:70vh}}
</style></head><body>
